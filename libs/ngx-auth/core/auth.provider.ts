...
export const AUTH_MANAGER: InjectionToken<OIDCAuthManager> = new InjectionToken<OIDCAuthManager>('AUTH_MANAGER');

export const initAuth = async (settings: AuthSettings): Promise<StaticProvider> => ({
    provide: AUTH_MANAGER,
    useValue: await initOidc(settings, '@badisi/ngx-auth'),
    multi: false
});
