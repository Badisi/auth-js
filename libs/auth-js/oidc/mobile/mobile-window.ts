...
import { type IWindow, Logger, type NavigateParams, type NavigateResponse } from 'oidc-client-ts';
...
export class MobileWindow implements IWindow {
    ...
    public async close(): Promise<void> {
         ...
         */
        if (!this._isClosed) {
            await CAPACITOR_BROWSER?.close().catch((err: unknown) => { logger.error(err); });
